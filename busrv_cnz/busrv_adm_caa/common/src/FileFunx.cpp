...
void FileFunx::chown(const char* path, const char* groupName)
{
    ...
    uid_t apgUserId = Config::getCPSUserId();
#endif

    struct passwd pwd_data;
    struct passwd *pwd_ptr = &pwd_data;
    struct passwd *pwd_res;
    char* buf_pwd = 0;
    size_t bufflen = sysconf(_SC_GETPW_R_SIZE_MAX);
    boost::scoped_array<char> tbuf_ptr1;
    tbuf_ptr1.reset(buf_pwd = new (nothrow) char[bufflen]);
    
    getpwnam_r("root", pwd_ptr, buf_pwd, bufflen, &pwd_res);

    char* buf_grnam = 0;
    bufflen = sysconf(_SC_GETGR_R_SIZE_MAX);
	boost::scoped_array<char> tbuf_ptr2;
	tbuf_ptr2.reset(buf_grnam = new (nothrow) char[bufflen]);
	struct group group_data;
	struct group *grp_ptr = &group_data;
	struct group *grp_res = 0;
	getgrnam_r(groupName, grp_ptr, buf_grnam, bufflen, &grp_res);

#ifdef _APGUSERSVC_
    // TODO: Clear CAP_SYS_RESOURCE on root
    ...
    // Change the owner & group id to apgusersvc/ FMSUSRGRP
    if (::chown(path, pwd_ptr->pw_uid, grp_ptr->gr_gid)) {
        int error = errno;
        ss << "failed with errno = " << error << " : " << strerror(error);
        TRACE((LOG_LEVEL_ERROR, "%s", 0, ss.str().c_str()));
        EventReporter::instance().write(ss.str().c_str());
        if (error == 122) {
            // Quota exceeded
            TRACE((LOG_LEVEL_ERROR, "Quota limit exceeded", 0));
            THROW_LINUX_MSG("Quota limit exceeded");
        }
    }
    else {
        ss << "successful";
...
#ifdef _APGUSERSVC_
    // TODO: Set CAP_SYS_RESOURCE on root
    if (cap_set_flag(cap, CAP_EFFECTIVE, 1, cap_list, CAP_SET) == -1)
         TRACE((LOG_LEVEL_ERROR, "cap_set_flag() Failed to remove CAP_SYS_RESOURCE to root user!", 0));
    if (cap_set_proc(cap))
          TRACE((LOG_LEVEL_ERROR, "cap_set_proc() Failed to remove CAP_SYS_RESOURCE to root user!", 0));

    if (cap_free(cap) == -1)
          TRACE((LOG_LEVEL_ERROR, "cap_free failed!", 0));
        // handle error ;

    // Set effective user as apg user id
    if( seteuid(apgUserId) != 0 )
    {    
...
size_t FileFunx::cleanDir(const char* dir, const char* regex_pattern, bool del_dir /* = true */)
{
    ...
    if (dp != NULL)
    {
        while ((ep = readdir(dp)) != NULL)
        {
            ...
            {
                TRACE((LOG_LEVEL_ERROR, "regex_match failed(%s, %s)", 0, buf, regex_pattern));
                continue;
            }
	...
	if (nofW32Words) {
		...
		for(u_int32 i = 0; i < loopCnt; ++i) {
			if(ReadFileX(fd, buf, BUF_SIZE)) {
				csum += buap::csumW32(reinterpret_cast<u_int32*>(buf), BUF_SIZE);
			}
			else {
		...
		if (lastBlockSize) {
			if(ReadFileX(fd, buf, lastBlockSize)) {
				csum += buap::csumW32(reinterpret_cast<u_int32*>(buf), lastBlockSize);
			}
			else {
				THROW_XCODE("Unexpected end-of-file error.", CodeException::BAD_DATA_FILE);
			}
